...
//        }
    ...
    * */
    public JSONArray finalllist(Pagebean pagebean ,User user){
     ...
     //   List list = jdbcTemplate.queryForList(sql.toString().replaceFirst("and","where")); //遇到第一个and就替换成where
        List list = jdbcTemplate.queryForList(sql.toString());//查询结果返回的是list集合
